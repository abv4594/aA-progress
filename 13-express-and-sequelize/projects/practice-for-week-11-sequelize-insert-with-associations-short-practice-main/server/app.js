...
app.post('/bands/:bandId/musicians', async (req, res, next) => {
    ...
    try {
        ...
        res.json({
            message: `Musician ${musician.id} succesfully created for Band ${band.id}`,
            musician: musician
        })
    } catch(err) {
        throw err
...
app.post('/musicians/:musicianId/instruments', async (req, res, next) => {
    ...
    const instruments = [];
    const instrumentsId = req.body.instrumentsId
    for (let i = 0; i < instrumentsId.length; i++ ) {
        instruments.push(await Instrument.findByPk(instrumentsId[i]));
    }
    console.log(instruments);
    await musician.addInstruments(instruments);
    res.json({
        message: `Associated ${musician.firstName} with instruments ${req.body.instrumentsId}`
    })
})
